...
class _AddIncomeScreenState extends ConsumerState<AddIncomeScreen> {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        await ref.read(incomeServiceProvider).addIncome({
          ...
          'income_date': DateTime.now().toIso8601String().split('T')[0],
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Add Income', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
        centerTitle: true,
      ),
      body: SafeArea(
  ...
  Widget _buildSourceSelector() {
    // Use the incomeSources map from income_service.dart
    // Keys are DB enum values: salary, freelance, business, etc.
